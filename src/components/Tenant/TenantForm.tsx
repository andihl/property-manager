...
const TenantForm = (props: Props): ReactElement => {
    ...
    const [dateOfBirth, setDateOfBirth] = useState(props.tenant?.dateOfBirth.toString().split('T')[0] || '');
    const [address, setAddress] = useState(props.tenant?.address || '');
    const [phone, setPhone] = useState(props.tenant?.phone || '');
    const [email, setEmail] = useState(props.tenant?.email || '');
